...
def export_experiment(
    ...
    if export_permissions:
        mlflow_attr["permissions"] = ws_permissions_utils.get_experiment_permissions(
            dbx_client, exp.experiment_id
        )
    io_utils.write_export_file(
        output_dir, "experiment.json", __file__, mlflow_attr, info_attr
    )
    ...
    if num_runs_exported == 0:
        ...
        _logger.info(
            f"{len(ok_run_ids)}/{num_runs_exported} runs succesfully exported {msg}"
        )
    ...
    for run_id in run_ids:
        try:
            ...
            if run.info.experiment_id == exp.experiment_id:
                ...
                _logger.warning(
                    f"Not exporting run since it doesn't belong to experiment: {msg}"
                )
                failed_run_ids.append(run.info.run_id)
